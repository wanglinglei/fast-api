...
import { Project, SyntaxKind, InterfaceDeclarationStructure, OptionalKind, EnumDeclarationStructure } from "ts-morph";
...
export class Group {
  ...
  writeDefaultImport({
    ...
    // 检查是否已经有该默认导入
    const existingImport = newApiFile.getImportDeclaration((importDeclaration: any) =>
  ...
  generateApiTemAndDefineTs(groupData: any[]) {
    ...
    groupData.map((element: GroupFile) => {
      ...
      //创建空模版文件
      const newApiFile = project.createSourceFile(targetFilePath, {}, { overwrite: true });
      ...
      if (variableDeclaration) {
        ...
        const tsTypeFile = project.createSourceFile(outputPath, {}, { overwrite: true });
        ...
        itemApis.map(apiPath => {
          ...
          const apiName = apiPath.method + (this.capitalizeFirstLetterWithRegex(apiAllStr[apiAllStr.length - 1]))
          ...
          const interfaceDeclaration: OptionalKind<InterfaceDeclarationStructure> = {
            name: interfaceName,
            isExported: true,
            properties: apiPath.params?.filter((f: any) => !f.isModal).map((prop: any) => ({
              name: prop.key,
              type: prop.type,
          ...
          //枚举定义
          const enumDataList = apiPath.params?.filter((l: any) => l.key == 'enumType')
          if (enumDataList && enumDataList.length) {
            enumDataList.forEach((enumData: any) => {
              const enumDeclaration: OptionalKind<EnumDeclarationStructure> = {
                name: enumData.key,
                isExported: true,
                members: enumData.enumKey.map((member: any) => ({ name: member, value: member, docs: [`${enumData?.enumDesc[member]}`] }))
              };
          ...
          const modalAll = apiPath.params.filter((f: any) => f.isModal).map(m => m.key)
          const modalType = modalAll.join('|')
          const uniteType = modalType.length ? modalType + `|${interfaceName}` : interfaceName
          ...
          initializer.addPropertyAssignment({
            ...
            initializer: `function (data:${uniteType}){
                                  ...
                                  url:'${apiPath.api}',
                                  ${apiPath.method == 'get' ? 'params:data' : 'data:data'}
                               })
                           },`
          })
